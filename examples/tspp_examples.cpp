...
void tspp_tests::test_lib()
{
    ...
    std::function<double(std::deque<double>)> xarrfunc = [](std::deque<double> buff) {
        return std::accumulate(buff.begin(), buff.end(), 0.0);
    };
    ...
    auto ts_ma_nm = ts_mal.merge_apply_asof<std::pair<double, double>>(ts_mar, std::numeric_limits<double>::quiet_NaN(),std::nullopt, pair_func, tspp::enums::merge_asof_mode::no_roll);
    ...
    auto comp_func = tspp::chrono_extensions::merge_helpers<std::chrono::high_resolution_clock::time_point>::merge_asof_prior<merge_itert, merge_itert>(std::chrono::seconds(1));
    auto ts_ma_p = ts_mal.merge_apply_asof<std::pair<double, double>>(ts_mar, std::numeric_limits<double>::quiet_NaN(), comp_func, pair_func, tspp::enums::merge_asof_mode::roll_prior);
    ...
    auto comp_func2 = tspp::chrono_extensions::merge_helpers<std::chrono::high_resolution_clock::time_point>::merge_asof_prior<merge_itert, merge_itert>(std::chrono::seconds(2));
    auto ts_ma_p2 = ts_mal.merge_apply_asof<std::pair<double, double>>(ts_mar, std::numeric_limits<double>::quiet_NaN(), comp_func2, pair_func, tspp::enums::merge_asof_mode::roll_prior);
    ...
    using merge_itert = typename tspp::time_series<double>::const_iterator;
    auto comp_funcfwd = tspp::chrono_extensions::merge_helpers<std::chrono::high_resolution_clock::time_point>::merge_asof_fwd<merge_itert, merge_itert>(std::chrono::seconds(1));
    auto ts_ma_f = ts_mal.merge_apply_asof<std::pair<double, double>>(ts_mar2, std::numeric_limits<double>::quiet_NaN(), comp_funcfwd, pair_func, tspp::enums::merge_asof_mode::roll_following);
