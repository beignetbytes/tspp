...
namespace tspp {
	...
	template <class T, class dt_type = std::chrono::high_resolution_clock::time_point, template <class> class backing_class = default_vec_t>
	class time_series {
	  public:
	  ...
	  public:
		...
		/*! copy constructor */
		time_series(const time_series<T, dt_type> &src)
			: time_series(src.keys_, src.values_){
		...
		bool empty() const { return keys_.empty() && values_.empty(); };
...
#pragma region reverse iterators
		...
		struct reverse {
			using const_reverse_iterator = typename tspp::iterator_extensions::iterator_pair<std::reverse_iterator<typename key_container::const_iterator>,
																							 std::reverse_iterator<typename value_container::const_iterator>>;
			using reverse_iterator = typename tspp::iterator_extensions::iterator_pair<std::reverse_iterator<typename key_container::iterator>,
																					   std::reverse_iterator<typename value_container::iterator>>;
		...
		template <typename _iter>
		using _is_rand_access = std::disjunction<std::is_same<typename std::iterator_traits<_iter>::iterator_category, std::random_access_iterator_tag>,
												 std::is_base_of<typename std::iterator_traits<_iter>::iterator_category, std::random_access_iterator_tag>>;

		template <typename _iter>
		using _is_native_reverseable = std::disjunction<_is_rand_access<_iter>, _is_bidir<_iter>>;

		using enable_reverse = typename std::conditional<std::conjunction<_is_native_reverseable<key_iterator>, _is_native_reverseable<value_iterator>>::value,
														 std::bidirectional_iterator_tag, std::input_iterator_tag>::type;
...
#pragma endregion
		...
		const_reverse_iterator crbegin() const { return reverser(keys_, values_).crbegin(); }
		const_reverse_iterator crend() const { return reverser(keys_, values_).crend(); }
	  ...
	  public:
		...
		time_series<out_value, dt_type, backing_class> cross_apply_inner(const time_series<other_value, dt_type, backing_class> &other,
																		 const cross_apply_func &func) const {
		...
		template <typename out_value, typename other_value, typename cross_apply_func>
		time_series<out_value, dt_type, backing_class> cross_apply_left(const time_series<other_value, dt_type, backing_class> &other,
																		other_value default_value, const cross_apply_func &func) const {
		...
		time_series<out_value, dt_type, backing_class> merge_apply_asof(
			const time_series<other_value, dt_type, backing_class> &other, other_value default_value,
			const std::optional<std::function<std::pair<tspp::enums::ordering, typename tspp::time_series<other_value, dt_type, backing_class>::const_iterator>(
				const_iterator, typename tspp::time_series<other_value, dt_type, backing_class>::const_iterator,
				typename tspp::time_series<other_value, dt_type, backing_class>::const_iterator)>> &compare_func,
			const apply_func &func, const tspp::enums::merge_asof_mode &merge_mode) const {
			...
			std::function<std::pair<tspp::enums::ordering, other_const_iterator>(const_iterator, other_const_iterator, other_const_iterator)> comp_func =
				compare_func.value_or([](const_iterator _this, other_const_iterator _other,
										 other_const_iterator _other_alt) -> std::pair<tspp::enums::ordering, other_const_iterator> {
		...
		time_series<value_type, dt_type, backing_class> interweave(const time_series<value_type, dt_type, backing_class> &other,
																   const selec_func &selecfn) const {
